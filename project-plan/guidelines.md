...
* It will use SQLAlchemy as the ORM to interact with a single, common PostgreSQL database.
* The application will expose RESTful APIs for querying and managing customer data.
* The PostgreSQL database will be initialized with a schema defined in `oltp-schema.sql`, which will include tables for customers, credit ratings, risk scores, and other related data.
...
#### DW Backend
* It will be a Docker containerized Python application using FastAPI as the web framework.
* It will use SQLAlchemy as the ORM to interact with the same single, common PostgreSQL database.
* The application will expose RESTful APIs for querying and managing customer data.
* The PostgreSQL database will be initialized with a schema defined in `dw-schema.sql`, which will include tables for customers, credit ratings, risk scores, and other related data.
* It will run migrations and load data using Alembic, with migration scripts stored in the `migrations/` directory.
* It will have an endpoint to run migrations, and another endpoint to load data. 
...
* It will use SQLAlchemy as the ORM to interact with the single PostgreSQL database (using separate schemas or tablespaces if needed for OLTP vs DW data).
* It will extract from the OLTP schema and load into the DW schema within the same PostgreSQL database.
  * It will not store the data in its own database, but only write to the DW schema.
  * It will store the IDs of the last processed records in a table to ensure that it can resume from where it left off previously.
...
* It will use SQLAlchemy as the ORM to interact with the single, common PostgreSQL database.
...
* It will include endpoints for user authentication, customer data retrieval, and data retrieval
  * The three endpoints will be: 
    * http://api-backend/customer_detail to retrieve customer details from the OLTP schema
      * It will accept a string "customer_name" as a query parameter, execute a hard-coded SQL query and return the customer details from the OLTP schema in the form of:
