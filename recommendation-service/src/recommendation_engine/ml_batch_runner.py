...
class MLBatchRunner:
    ...
    def setup_connections(self, mongo_config: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            # Initialize ML MongoDB loader with environment variable fallbacks
            self.ml_loader = MLMongoDBLoader(
                db_connector=self.db_connector,
                mongo_host=mongo_config.get('host', os.environ.get("MONGO_HOST", "localhost")),
                mongo_port=mongo_config.get('port', int(os.environ.get("MONGO_PORT", 27017))),
                mongo_database=mongo_config.get('database', os.environ.get("MONGO_DATABASE", "askdata_mongo")),
                mongo_username=mongo_config.get('username', os.environ.get("MONGO_INITDB_ROOT_USERNAME")),
                mongo_password=mongo_config.get('password', os.environ.get("MONGO_INITDB_ROOT_PASSWORD"))
            )
...
def parse_arguments():
    ...
    # MongoDB configuration - Read from environment variables by default
    parser.add_argument('--mongo-host',
                       default=os.environ.get("MONGO_HOST", "localhost"),
                       help='MongoDB host (default: from MONGO_HOST env or localhost)')

    parser.add_argument('--mongo-port',
                       type=int,
                       default=int(os.environ.get("MONGO_PORT", 27017)),
                       help='MongoDB port (default: from MONGO_PORT env or 27017)')

    parser.add_argument('--mongo-database',
                       default=os.environ.get("MONGO_DATABASE", "askdata_mongo"),
                       help='MongoDB database (default: from MONGO_DATABASE env or askdata_mongo)')

    parser.add_argument('--mongo-username',
                       default=os.environ.get("MONGO_INITDB_ROOT_USERNAME"),
                       help='MongoDB username (default: from MONGO_INITDB_ROOT_USERNAME env)')

    parser.add_argument('--mongo-password',
                       default=os.environ.get("MONGO_INITDB_ROOT_PASSWORD"),
                       help='MongoDB password (default: from MONGO_INITDB_ROOT_PASSWORD env)')
